...
- 2026-05-26 #69: The PR body test-plan AC1 cited file:TestName (no line number) while ACs 2-4 had numeric lines — enforcing file:line strictly in the spec-conformance pass would have flagged this; reviewers should accept function-name citations as navigable equivalents only when the test name is unique in the file.
- 2026-05-26 #70: The PasswordHash field on the User struct is never leaked because endpoints project to userDTO, but adding json:"-" to the field would be zero-cost defence-in-depth — plan for it on any domain struct that mixes PII storage with JSON serialization.
- 2026-05-26 #71: The authStore module-scope re-hydration try/catch on load (not just on setItem/removeItem) is the correct pattern for any store that reads browser globals at import time — plan for it upfront on any frontend store that touches localStorage, sessionStorage, or indexedDB.
- 2026-05-26 #72: The 'What needs to happen' prose in issue descriptions can include requirements (payload validation) not reflected in any AC checkbox — reviewers should flag missing AC coverage as a suggestion, not a blocker, when the requirement is prose-only and not a checkboxed criterion.
- 2026-05-26 #73: The cancelled-flag pattern in useEffect (set in cleanup) is the correct guard against stale setState on unmount for any async data-fetch effect — plan for it upfront on every page that fetches on mount.
- 2026-05-26 #74: Docs-only PRs with no code changes still benefit from a DB-label convention (DB vs no-DB) in verification commands — adding this upfront during initial file creation would have made the smoke-tester's job deterministic without a FIX cycle.
